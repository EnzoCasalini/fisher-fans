...
def get_log_page(user_id: str = Path(...), page_id: str = Path(...), db: Session = Depends(get_db)) -> PydanticPage:
    ...
    """
    page = db.query(SQLAlchemyPage).filter(SQLAlchemyPage.id == page_id,
                                           SQLAlchemyPage.log.has(user_id=user_id)).first()
    if not page:
...
def update_log_page(
    ...
    """
    db_page = db.query(SQLAlchemyPage).filter(SQLAlchemyPage.id == page_id,
                                              SQLAlchemyPage.log.has(user_id=user_id)).first()
    if not db_page:
        raise HTTPException(status_code=404, detail="Page not found")
